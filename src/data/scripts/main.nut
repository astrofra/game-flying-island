...
class	MainProject
{
	...
	scene_list 		= [	{	name = "Language selection",	nms = "data/language.nms"},
						...
						{	name = "~~Size(60)Level 1,\n~~Size(70)the hungry sea",		nms = "data/level_intro.nms"},
						{	name = "Level #1",				nms = "data/boat.nms"}
					]
	...
	function	OnUpdate(project)
	{
		...
		{	
			...
			scene_2d = ProjectInstantiateScene(g_project, "data/blank_ui.nms")
			ProjectAddLayer(g_project, scene_2d, -1.0)
			g_ui = ProjectSceneGetInstance(scene_2d)

			//	Load next scene
			current_scene = next_scene
			print("MainProject::OnUpdate() : Loading scene " + scene_list[current_scene].name)
			scene_3d = ProjectInstantiateScene(g_project, scene_list[current_scene].nms)
			ProjectAddLayer(g_project, scene_3d, 1.0)
